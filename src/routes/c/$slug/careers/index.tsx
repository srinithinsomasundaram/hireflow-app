...
export const Route = createFileRoute("/c/$slug/careers/")({
  ...
  head: ({ loaderData }) => {
    ...
    const slug  = loaderData?.org.slug ?? "";
    ...
    const canonicalUrl = `https://${appDomain}/c/${slug}/careers`;

    // Geo from first job location (default to India)
    const firstLoc = loaderData?.jobs?.[0]?.location ?? "";
    const isIndia = !firstLoc ||
      /india|mumbai|delhi|bangalore|bengaluru|chennai|hyderabad|pune|kolkata|ahmedabad|jaipur|lucknow/i.test(firstLoc);
    const geoRegion    = isIndia ? "IN" : "US";
    const geoPlacename = firstLoc ? firstLoc.split(",").pop()?.trim() ?? (isIndia ? "India" : "") : (isIndia ? "India" : "");
    ...
    const orgSchema = {
      ...
      "@type": "Organization",
      "name": name,
      "url": loaderData?.org.website ?? canonicalUrl,
      ...(image ? { "logo": image } : {}),
      ...(loaderData?.org.website ? { "sameAs": [loaderData.org.website] } : {}),
    };

    // JSON-LD: ItemList of job postings
    const itemListSchema = loaderData?.jobs?.length ? {
      "@context": "https://schema.org",
      "@type": "ItemList",
      "name": `Open positions at ${name}`,
      "url": canonicalUrl,
      "numberOfItems": loaderData.jobs.length,
      "itemListElement": loaderData.jobs.map((j, i) => ({
        "@type": "ListItem",
        "position": i + 1,
        "url": `https://${appDomain}/c/${slug}/careers/jobs/${j.id}`,
        "name": j.title,
      })),
    } : null;
    ...
    return {
      ...
      links: [
        ...
        ...(loaderData?.org.logo_url ? [{ rel: "icon", href: loaderData.org.logo_url }] : []),
      ],
      scripts: [
        { type: "application/ld+json", children: JSON.stringify(orgSchema) },
        ...(itemListSchema ? [{ type: "application/ld+json", children: JSON.stringify(itemListSchema) }] : []),
      ],
    };
  },
    ...
    <div className="min-h-screen" style={{ background: pageBg, fontFamily: fontStack(bodyFont) }}>
      ...
      <footer className="border-t bg-white mt-6">
        <div className="mx-auto max-w-4xl px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs text-muted-foreground">
          ...
          {showPowered && (
            <a href="https://hireflow.yesp.space" target="_blank" rel="noopener" className="opacity-60 hover:opacity-90 transition-opacity">
              Powered by HireFlow
            </a>
          )}
        </div>
