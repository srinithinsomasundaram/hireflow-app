...
export const Route = createFileRoute("/c/$slug/careers/jobs/$jobId")({
  ...
  head: ({ loaderData, params }) => {
    type JobRow = {
      title: string; description: string | null; department: string | null;
      location: string | null; employment_type: string;
      salary_min: number | null; salary_max: number | null; salary_currency: string | null;
      published_at: string | null;
      organizations: { company_name: string; logo_url: string | null; website?: string | null; slug: string };
    };
    const job = loaderData as unknown as JobRow | null;
    const org   = job?.organizations ?? null;
    const title = job ? `${job.title} · ${org?.company_name ?? "Careers"}` : "Job · Careers";
    const desc  = job?.description?.slice(0, 160)
      ?? (job ? `Apply for ${job.title}${org?.company_name ? ` at ${org.company_name}` : ""}.` : "Apply now");
    const image = org?.logo_url ?? null;

    const appDomain   = import.meta.env.VITE_APP_DOMAIN ?? "hireflow.yesp.space";
    const slugParam   = params.slug;
    const canonicalUrl = `https://${appDomain}/c/${slugParam}/careers/jobs/${params.jobId}`;

    // Geo from job location
    const loc      = job?.location ?? "";
    const isIndia  = !loc || /india|mumbai|delhi|bangalore|bengaluru|chennai|hyderabad|pune|kolkata|ahmedabad|jaipur|lucknow/i.test(loc);
    const geoRegion    = isIndia ? "IN" : "US";
    const geoPlacename = loc ? loc.split(",").pop()?.trim() ?? (isIndia ? "India" : "") : (isIndia ? "India" : "");
    ...
    const jobPostingSchema = job ? {
      ...
      "@type": "JobPosting",
      "title": job.title,
      "description": job.description ?? `Apply for ${job.title} at ${org?.company_name ?? "this company"}.`,
      "datePosted": job.published_at ?? new Date().toISOString().slice(0, 10),
      ...
      "jobLocation": {
        ...
        "address": {
          "@type": "PostalAddress",
          "addressLocality": loc.split(",")[0]?.trim() || geoPlacename,
          "addressCountry": geoRegion,
        },
      },
    ...
    return {
      meta: [
        ...
        { name: "generator",       content: "HireFlow" },
      ...
      links: [
        ...
        ...(org?.logo_url ? [{ rel: "icon", href: org.logo_url }] : []),
      ],
      ...(jobPostingSchema ? {
        scripts: [{ type: "application/ld+json", children: JSON.stringify(jobPostingSchema) }],
      } : {}),
    };
  },
  component: JobPublic,
    ...
    <div className="min-h-screen bg-background">
      ...
      <footer className="mt-10 border-t">
        <div className="mx-auto max-w-3xl px-6 py-6 text-xs text-muted-foreground">
          <a href="https://hireflow.yesp.space" target="_blank" rel="noopener" className="hover:opacity-70 transition-opacity">
            Powered by HireFlow
          </a>
        </div>
      </footer>
