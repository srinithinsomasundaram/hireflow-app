...
function ResetPassword() {
  ...
  // If no recovery token in the URL, mark invalid immediately — no point waiting.
  const [ready, setReady] = useState(false);
  const [invalid, setInvalid] = useState(() => !hasRecoveryToken());
  ...
  useEffect(() => {
    if (invalid) return; // no token — nothing to listen for
    ...
    // Also cover page-refresh after recovery event already fired
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) setReady(true);
    });

    // If no recovery event fires within 4 seconds the link is expired
