...
import {
  ...
  Layers, ImageIcon, X, Loader2, RefreshCcw, Briefcase, LogOut,
} from "lucide-react";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";
import { setActiveOrgId } from "@/hooks/use-current-org";
...
function Onboarding() {
  ...
  async function onSubmitBrand(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const wsSlug = slugify(slug || workspaceName);

      // 1. Resolve the parent organisation — reuse an existing one if the user
      //    already completed org creation in a previous (partial) attempt.
      ...
      const { data: existingRoles } = await supabase
        .from("user_roles")
        .select("organizations(id, parent_org_id)")
        .eq("user_id", user.id)
        .eq("status", "active");

      const existingParentRole = (existingRoles ?? []).find((r) => {
        const row = r as unknown as { organizations: { id: string; parent_org_id: string | null } | null };
        return row.organizations?.parent_org_id === null;
      });

      if (existingParentRole) {
        const row = existingParentRole as unknown as { organizations: { id: string } };
        orgId = row.organizations.id;
      } else {
        // Org slug is internal-only (never shown in URLs) — use a random suffix to
        // guarantee uniqueness without a server round-trip that can fail silently.
      ...
      // 2. Create the first Workspace (brand/sub-company) under the organisation
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: wsId, error: wsErr } = await (supabase as any).rpc("create_workspace", {
      ...
      // 4. Set workspace subdomain for the careers page
      if (wsId) {
        await supabase.from("organizations").update({ subdomain: wsSlug }).eq("id", wsId);
      }

      // 5. Upload logo to the workspace
      if (favicon && wsId) {
        const ext = favicon.name.split(".").pop()?.toLowerCase() ?? "png";
        const path = `${wsId}/favicon.${ext}`;
        const { error: upErr } = await supabase.storage
          .from("logos")
          .upload(path, favicon, { upsert: true, contentType: favicon.type });
        if (!upErr) {
          const { data: { publicUrl } } = supabase.storage.from("logos").getPublicUrl(path);
          await supabase.from("organizations").update({ logo_url: publicUrl }).eq("id", wsId);
        }
      }

      toast.success("Organisation & workspace created — welcome to HireFlow!");
      // Pin the new workspace so useCurrentOrg resolves it immediately on navigation
      if (wsId) setActiveOrgId(String(wsId));
            ...
            <div>
              <div className="mb-6">
                ...
                <p className="mt-1.5 text-sm text-gray-500">
                  Your company — the top-level entity. All brands and divisions will live under it.
                </p>
              </div>
          ...
          {step === "workspace" && (
            <div>
              <div className="mb-6">
                ...
                <p className="mt-1.5 text-sm text-gray-500">
                  A workspace is a brand or division under your organisation — with its own careers page, pipeline, and team.
                </p>
              </div>
