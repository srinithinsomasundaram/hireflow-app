...
function AuthenticatedLayout() {
  ...
  const { data: org, isFetched: orgFetched, isFetching: orgFetching } = useCurrentOrg();

  // Redirect to onboarding when session exists but user has no org membership.
  // Guard !orgFetching so we don't redirect mid-refetch (e.g. after org creation
  // invalidates the cache — data is briefly stale-null before the fresh fetch lands).
  useEffect(() => {
    if (orgFetched && !orgFetching && org === null) {
      navigate({ to: "/onboarding" });
    }
  }, [org, orgFetched, orgFetching, navigate]);
  ...
  if (!orgFetched || (orgFetching && org === null)) return <PageLoader />;
