...
const TRIGGERS = [
  ...
  { value: "ai_score_calculated",     label: "AI score calculated" },
];
const ACTIONS = [
...
type TemplateCategory = "email" | "notifications" | "pipeline" | "onboarding" | "ai_screening";
...
const TEMPLATES: AutomationTemplate[] = [
  ...
  // ── Onboarding ──────────────────────────────────────────────────────────────
  { id: "onboarding-welcome-email", name: "Welcome to the team email",
    description: "Send a warm welcome email to the candidate when they're marked as Hired — sets expectations before the onboarding portal invite arrives.",
    trigger: "stage_changed", trigger_config: { stage_filter: "hired" }, action: "send_email",
    action_config: { delay_minutes: 0, subject: "Welcome to {{company_name}}! 🎉", message: "Dear {{candidate_name}},\n\nWe are absolutely delighted to welcome you to {{company_name}}! We are thrilled to have you joining our team for the {{job_title}} position.\n\nIn the coming days, you will receive a dedicated onboarding portal link to help you complete the necessary paperwork and documentation before your first day. Please keep an eye on your inbox.\n\nIn the meantime, should you have any questions, please don't hesitate to reach out to your HR contact.\n\nWe look forward to having you on board.\n\nWarm regards,\n{{company_name}} HR Team" },
    category: "onboarding", popular: true },

  { id: "onboarding-hr-notify", name: "Notify HR to initiate onboarding",
    description: "Alert the HR team the moment a candidate is marked Hired so they can create the onboarding session without delay.",
    trigger: "stage_changed", trigger_config: { stage_filter: "hired" }, action: "notify_team",
    action_config: { delay_minutes: 0, message: "{{candidate_name}} has been marked as Hired for {{job_title}}. Please create their onboarding session from Employee Onboarding and send the portal invite." },
    category: "onboarding", popular: true },

  { id: "onboarding-portal-reminder", name: "Onboarding portal reminder",
    description: "Remind the new hire to complete their onboarding portal 3 days after being hired — in case the invite email was missed.",
    trigger: "stage_changed", trigger_config: { stage_filter: "hired" }, action: "send_email",
    action_config: { delay_minutes: 4320, subject: "Reminder: Complete your onboarding — {{company_name}}", message: "Dear {{candidate_name}},\n\nThis is a friendly reminder that your onboarding portal is ready and waiting for you. Completing it before your first day helps us make sure everything is set up smoothly on your end.\n\nIf you have not yet received the portal link, please contact your HR team and they will resend it to you right away.\n\nWe look forward to seeing you soon!\n\nKind regards,\n{{company_name}} HR Team" },
    category: "onboarding" },

  { id: "onboarding-docs-team-notify", name: "Onboarding documents submitted — notify HR",
    description: "Alert HR the moment a candidate submits their onboarding form so they can review documents without delay.",
    trigger: "onboarding_submitted", trigger_config: {}, action: "notify_team",
    action_config: { delay_minutes: 0, message: "{{candidate_name}} has completed and submitted their onboarding form for {{job_title}}. Please log in to Employee Onboarding to review their submitted documents and information." },
    category: "onboarding", popular: true },

  { id: "onboarding-submitted-candidate", name: "Onboarding submission confirmation",
    description: "Confirm to the candidate that their onboarding documents have been received and are under review.",
    trigger: "onboarding_submitted", trigger_config: {}, action: "send_email",
    action_config: { delay_minutes: 0, subject: "Onboarding complete — {{company_name}}", message: "Dear {{candidate_name}},\n\nThank you for completing your onboarding submission for the {{job_title}} role at {{company_name}}. We have received all your details and documents successfully.\n\nOur HR team will review your submission and reach out if any additional information is required. We are excited to have you joining us!\n\nKind regards,\n{{company_name}} HR Team" },
    category: "onboarding" },

  { id: "onboarding-auto-tag", name: "Tag new hires for onboarding tracking",
    description: "Automatically add an 'onboarding-pending' tag when a candidate is hired — makes it easy to filter and track onboarding status.",
    trigger: "stage_changed", trigger_config: { stage_filter: "hired" }, action: "add_tag",
    action_config: { delay_minutes: 0, message: "onboarding-pending" },
    category: "onboarding" },

  // ── AI Screening ────────────────────────────────────────────────────────────
  { id: "ai-score-alert", name: "New applicant AI score alert",
    description: "Notify the hiring team of each new applicant and prompt them to check the AI fit score before deciding on next steps.",
    trigger: "application_received", trigger_config: {}, action: "notify_team",
    action_config: { delay_minutes: 0, message: "New application from {{candidate_name}} for {{job_title}} has been received and AI-scored. Please log in to the Candidates section to review the fit score and decide on the next step." },
    category: "ai_screening", popular: true },

  { id: "ai-auto-screen", name: "Auto-advance all applicants to screening",
    description: "Move every new application to Screening instantly so your team can review AI scores in one focused batch session.",
    trigger: "application_received", trigger_config: {}, action: "move_stage",
    action_config: { delay_minutes: 0, message: "screening" },
    category: "ai_screening", popular: true },

  { id: "ai-tag-to-screen", name: "Tag applicants for AI review",
    description: "Add a 'to-screen' tag to every new application so recruiters can filter and work through them in one go.",
    trigger: "application_received", trigger_config: {}, action: "add_tag",
    action_config: { delay_minutes: 0, message: "to-screen" },
    category: "ai_screening" },

  { id: "ai-screening-reminder", name: "Screening stage review reminder",
    description: "Remind the team to check AI scores and move candidates along once they reach the Screening stage.",
    trigger: "stage_changed", trigger_config: { stage_filter: "screening" }, action: "notify_team",
    action_config: { delay_minutes: 60, message: "{{candidate_name}} has moved to the Screening stage for {{job_title}}. Please review their AI fit score in the Candidates section and schedule a screening call if the score meets your threshold." },
    category: "ai_screening" },

  { id: "ai-screening-ack", name: "Screening acknowledgement to candidate",
    description: "Let candidates know their application is actively being reviewed once they reach Screening — sets a positive tone and reduces drop-off.",
    trigger: "stage_changed", trigger_config: { stage_filter: "screening" }, action: "send_email",
    action_config: { delay_minutes: 0, subject: "Application update — {{job_title}} | {{company_name}}", message: "Dear {{candidate_name}},\n\nThank you for your interest in the {{job_title}} role at {{company_name}}. We are pleased to let you know that your application has progressed to the screening stage and is currently under active review by our hiring team.\n\nA member of our team will be in touch shortly to discuss potential next steps. In the meantime, please feel free to reach out if you have any questions.\n\nKind regards,\n{{company_name}} Talent Acquisition" },
    category: "ai_screening" },

  { id: "ai-hr-interview-notify", name: "Prompt team to score before HR interview",
    description: "Remind interviewers to review the AI score report before the HR interview round for more informed conversations.",
    trigger: "stage_changed", trigger_config: { stage_filter: "hr_interview" }, action: "notify_team",
    action_config: { delay_minutes: 0, message: "{{candidate_name}} has advanced to the HR Interview stage for {{job_title}}. Please review their AI scoring report in HireFlow before the interview to prepare targeted questions." },
    category: "ai_screening" },
];
...
const CATEGORY_META = {
  ...
  onboarding:    { label: "Onboarding",    bar: "bg-indigo-500",  text: "text-indigo-700",  bg: "bg-indigo-50",  border: "border-indigo-200" },
  ai_screening:  { label: "AI Screening",  bar: "bg-amber-500",   text: "text-amber-700",   bg: "bg-amber-50",   border: "border-amber-200" },
};
...
function Automations() {
  ...
  const avail = (cat?: TemplateCategory) =>
    TEMPLATES.filter(t => !usedPairs.has(`${t.trigger}:${t.action}`) && (!cat || t.category === cat)).length;
  const counts = {
    all:           avail(),
    email:         avail("email"),
    notifications: avail("notifications"),
    pipeline:      avail("pipeline"),
    onboarding:    avail("onboarding"),
    ai_screening:  avail("ai_screening"),
  };
        ...
        <>
          ...
          <section className="space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {(["all", "onboarding", "ai_screening", "email", "notifications", "pipeline"] as const).map(cat => {
