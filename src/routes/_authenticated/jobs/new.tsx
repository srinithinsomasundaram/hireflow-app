...
import { ChevronRight, Loader2, Globe, Sparkles, X, ArrowRight } from "lucide-react";
...
function AiBar({ onFill }: { onFill: (form: typeof DEFAULT_FORM) => void }) {
  ...
  async function generate() {
    ...
    setLoading(true);
    try {
      const job = await generateJobFromPrompt({ data: { prompt: p } });
      onFill({
        title:           job.title ?? "",
      ...
      toast.success("Job generated — review and publish when ready");
      setOpen(false);
      setPrompt("");
    } catch (e) {
  ...
  return (
    <>
      ...
      {/* Floating bar (slides up above the button) */}
      {open && (
        <div className="fixed bottom-20 right-6 z-50 w-[min(480px,calc(100vw-3rem))] rounded-2xl border bg-background shadow-2xl ring-1 ring-black/5 overflow-hidden animate-in slide-in-from-bottom-3 fade-in duration-150">
          {/* Header */}
          <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b bg-muted/30">
            <Sparkles className="h-4 w-4 text-primary shrink-0" />
            <p className="text-sm font-semibold flex-1">Describe the role</p>
          ...
          <div className="p-3 space-y-3">
            <div className="flex gap-2">
              <Input
                ref={inputRef}
                placeholder='e.g. "Senior React dev, remote, 15–20 LPA"'
                value={prompt}
              ...
              />
              <Button size="sm" onClick={generate} disabled={loading || !prompt.trim()} className="gap-1.5 shrink-0">
                {loading
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                "Sales exec, Chennai, 6–9 LPA",
                "Full-stack engineer, remote",
                "HR manager, Bangalore, 8–12 LPA",
                "Product designer, contract",
              ].map(ex => (
                <button
                  key={ex}
                  onClick={() => { setPrompt(ex); inputRef.current?.focus(); }}
                  className="rounded-full border bg-muted/40 px-2.5 py-0.5 text-xs text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
                >
                  {ex}
                </button>
              ))}
            ...
            <p className="text-[11px] text-muted-foreground/60">Press Enter to generate · Esc to close</p>
          </div>
        </div>
      )}
    </>
        ...
        <div className="space-y-4">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-4 pt-4">
              <div>
                <Label htmlFor="t">Job title <span className="text-destructive">*</span></Label>
                <Input id="t" className="mt-1.5" required value={form.title}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <Input id="dept" className="mt-1.5" value={form.department}
                    onChange={e => setForm({ ...form, department: e.target.value })}
                    placeholder="e.g. Engineering" />
                </div>
                <div>
                  <Label htmlFor="loc">Location</Label>
                  <Input id="loc" className="mt-1.5" value={form.location}
              ...
              <div>
                ...
                <Select value={form.employment_type}
                  ...
                  <SelectTrigger className="mt-1.5 w-full sm:w-52"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <CardContent className="pt-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <Input id="smin" type="number" className="mt-1.5" value={form.salary_min}
                    onChange={e => setForm({ ...form, salary_min: e.target.value })} placeholder="800,000" />
                </div>
                <div>
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-4 pt-4">
              <div>
                ...
                <Textarea id="desc" rows={7} className="mt-1.5 resize-none" value={form.description}
                  onChange={e => setForm({ ...form, description: e.target.value })}
                  placeholder="What this role is about, what success looks like, team context…" />
              </div>
              <div>
                <Label htmlFor="req">Requirements</Label>
                <Textarea id="req" rows={5} className="mt-1.5 resize-none" value={form.requirements}
          ...
          <Card className="shadow-sm sticky top-4">
            ...
            <CardContent className="pt-4 space-y-4">
              ...
              <div className="space-y-2 pt-1">
                <Button className="w-full gap-1.5" disabled={!form.title || create.isPending}
                  onClick={() => create.mutate(true)}>
                  {create.isPending && <Loader2 className="h-4 w-4 animate-spin" />}
                  Publish now
                </Button>
                <Button variant="outline" className="w-full gap-1.5"
                  disabled={!form.title || create.isPending} onClick={() => create.mutate(false)}>
                  {create.isPending && <Loader2 className="h-4 w-4 animate-spin" />}
                  Save as draft
                </Button>
              </div>
