...
const main = [
  ...
  { title: "Onboarding", url: "/employee-onboarding", icon: ClipboardList },
  { title: "Talent CRM", url: "/crm", icon: Star },
];

const tools = [
  { title: "AI Screening", url: "/ai/screening", icon: Sparkles },
...
export function CreateWorkspaceDialog({
  ...
  async function onSubmitBrand(e: React.FormEvent) {
    ...
    try {
      ...
      // Find the user's parent organisation to create the workspace under
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) throw new Error("Not authenticated");

      // Try to find the parent org using parent_org_id (post-migration)
      // Fall back to using the oldest org (pre-migration)
      ...
      type OrgRef = { id: string; parent_org_id?: string | null };
      const allOrgs = (res.data ?? []).map((r) => {
        const row = r as unknown as { organizations: OrgRef | null };
        return row.organizations;
      }).filter(Boolean) as OrgRef[];

      // Find the parent org: either the one with no parent_org_id, or the first one
      const parentOrg = allOrgs.find(o => o.parent_org_id === null) ?? allOrgs[0];
      if (!parentOrg) throw new Error("No organisation found. Please complete onboarding first.");

      // Create the workspace under the parent org
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: wsId, error: wsErr } = await (supabase as any).rpc("create_workspace", {
        _parent_org_id: parentOrg.id,
      ...
      if (wsId) {
        await supabase.from("organizations").update({ subdomain: finalSlug }).eq("id", wsId);
      }
      ...
      await qc.invalidateQueries({ queryKey: ["org-hierarchy"] });
      if (wsId) switchOrg(wsId);
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); }}>
      <DialogContent className="sm:max-w-lg p-0 overflow-hidden">
        ...
        <div className="px-6 py-6">
          ...
          {step === "details" && (
            <div>
              <h2 className="text-xl font-bold tracking-tight text-gray-900">New workspace</h2>
              <p className="mt-1 text-sm text-gray-500">A brand or division under your organisation — with its own careers page and hiring pipeline.</p>
