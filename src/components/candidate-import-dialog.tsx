...
export function CandidateImportDialog({ open, onClose, onImported }: Props) {
  ...
  async function doImport() {
    ...
    try {
      ...
      const toInsert = validRows.filter(r => !existingMap.has(r.email.toLowerCase()));
      ...
      if (toInsert.length > 0) {
        ...
        newCandidateIds = (inserted ?? []).map(c => c.id);
        (inserted ?? []).forEach(c => existingMap.set(c.email.toLowerCase(), c.id));
      }

      // Create applications for newly inserted candidates if a job was selected
      if (selectedJobId && newCandidateIds.length > 0) {
      ...
      setImportResult({ inserted: toInsert.length, skipped: existingMap.size - newCandidateIds.length });
      onImported();
    } catch {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col gap-0 p-0">
                ...
                <div className="flex flex-col items-center justify-center gap-3 py-10 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {importResult.inserted} candidate{importResult.inserted !== 1 ? "s" : ""} added to the pipeline
                ...
                <>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="ml-auto flex items-center gap-1.5 shrink-0">
                      <span className="text-xs text-muted-foreground whitespace-nowrap">Job<span className="text-red-500">*</span></span>
                      <Select value={selectedJobId} onValueChange={setSelectedJobId}>
                        ...
                        <SelectContent>
                          {jobs.length === 0
                            ? <SelectItem value="none" disabled className="text-xs text-muted-foreground">No published jobs</SelectItem>
                            : jobs.map(j => (
        ...
        <DialogFooter className="px-5 py-3 border-t bg-muted/10 shrink-0 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "preview" && !importResult && (
              <Button size="sm" onClick={doImport} disabled={validRows.length === 0 || importing || !selectedJobId} className="gap-1">
                {importing
